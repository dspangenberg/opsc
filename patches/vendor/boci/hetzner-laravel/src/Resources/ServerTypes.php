<?php

namespace Boci\HetznerLaravel\Resources;

use Boci\HetznerLaravel\Contracts\ResourceContract;
use Boci\HetznerLaravel\Requests\ServerTypes\ListRequest;
use Boci\HetznerLaravel\Requests\ServerTypes\RetrieveRequest;
use Boci\HetznerLaravel\Responses\ServerTypes\ListResponse;
use Boci\HetznerLaravel\Responses\ServerTypes\RetrieveResponse;
use GuzzleHttp\ClientInterface;

/**
 * Server Types Resource
 *
 * This resource class provides methods for managing server types
 * in the Hetzner Cloud API.
 */
final class ServerTypes implements ResourceContract
{
    /**
     * Create a new server types resource instance.
     *
     * @param  ClientInterface  $httpClient  The HTTP client for API requests
     */
    public function __construct(
        private readonly ClientInterface $httpClient,
    ) {}

    /**
     * List all server types with optional filtering.
     *
     * @param  array<string, mixed>  $parameters  Optional query parameters for filtering
     */
    public function list(array $parameters = []): ListResponse
    {
        $request = new ListRequest($parameters);

        $response = $this->httpClient->request($request->method(), $request->uri(), $request->options());

        return ListResponse::from($response, $request);
    }

    /**
     * Retrieve a specific server type by ID.
     *
     * @param  string  $serverTypeId  The ID of the server type to retrieve
     */
    public function retrieve(string $serverTypeId): RetrieveResponse
    {
        $request = new RetrieveRequest($serverTypeId);

        $response = $this->httpClient->request($request->method(), $request->uri(), $request->options());

        return RetrieveResponse::from($response, $request);
    }
}
