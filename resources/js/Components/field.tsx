...
const fieldGroupStyles = tv({
  base: [
    ...
    "group flex h-10 items-center overflow-hidden rounded-sm border border-input shadow-xs transition duration-200 ease-out",
  ...
  return (
    <InputPrimitive
      ...
      className={composeTailwindRenderProps(
        className,
        "w-full min-w-0 bg-transparent px-(--gutter-x) py-1 text-base text-fg placeholder-muted-fg outline-hidden focus:outline-hidden sm:text-sm/6 [&::-ms-reveal]:hidden [&::-webkit-search-cancel-button]:hidden",
      )}
    />
  )
}
