...
interface Props {
  entries: App.Data.TimelineEntryData[]
  route: RouteUrl
}
...
export const TimelineView: FC<Props> = ({ entries, route: storeRoute }) => {
      ...
      <div className="mt-6 flex flex-1 flex-col items-start">
        <div className="flex w-full flex-col">
          {entries.map((item, index) => (
            <Fragment key={index}>
              {item.type === 'note' ? (
                <TimelineNote key={index} item={item.note as App.Data.NoteableData} />
              ) : (
                <TimelineEmail key={index} item={item.mail as App.Data.DropboxMailData} />
              )}
