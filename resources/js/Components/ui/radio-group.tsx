...
const RadioGroup = React.forwardRef(
  <T,>({ className, ...props }: RadioGroupProps<T>, ref: React.Ref<HTMLDivElement>) => {
    return (
...
const RadioGroupItem = React.forwardRef(
  <T,>({ className, ...props }: RadioGroupItemProps<T>, ref: React.Ref<HTMLButtonElement>) => {
    return (
      <RadioGroupPrimitive.Item
        ref={ref}
        className={cn(
          'aspect-square h-4 w-4 rounded-full border border-input shadow-sm shadow-black/5 disabled:cursor-not-allowed disabled:opacity-50 data-[state=checked]:border-primary data-[state=checked]:bg-primary/90 data-[state=checked]:text-white',
          focusInput,
          className
        )}
