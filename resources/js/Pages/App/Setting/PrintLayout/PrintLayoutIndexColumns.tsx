...
const deleteSection = async (row: App.Data.PrintLayoutData) => {
  const promise = await AlertDialog.call({
    ...
    message: `Möchtest Du das Layout ${row.title} wirklich löschen?`,
...
const RowActions = ({ row }: { row: Row<App.Data.PrintLayoutData> }) => {
  return (
...
export const columns: ColumnDef<App.Data.PrintLayoutData>[] = [
  {
    id: 'select',
    size: 30,
    header: ({ table }) => (
  ...
  {
    accessorKey: 'letterhead_id',
    header: 'Briefbogen',
    size: 300,
    cell: ({ row }) => <span>{row.original.letterhead?.title}</span>
  },
  {
    id: 'actions',
    size: 30,
