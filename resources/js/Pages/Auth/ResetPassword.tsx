...
const InitialPassword: React.FC<LoginProps> = ({ email, token }) => {
  const form = useForm<PasswordChangeProps>('auth-login-form', 'post', route('password.store', {}, false), {
    email,
    token,
    password: '',
  ...
  const loginContent = (
    <AuthContainer
      ...
      cardTitle="Neues Kennwort festlegen"
      cardDescription="Bitte wähle ein sicheres Kennwort für Deinen Account."
    >
      <Form form={form}>
        <FormGrid>
          <input
